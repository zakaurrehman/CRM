...
import { localiseStream } from "@/lib/i18n/content";
 ...
 */
export async function RecyclingStory() {
  const p = await getP();
  ...
  const featured = recoveryStreams.slice(0, 15).map((s) => localiseStream(s, locale));

  return (
    <Section tone="light">
      <SectionHeader
        eyebrow={p("Recycling & recovery")}
        title={p("Metal that would have gone to landfill, returned to the melt.")}
        description={p("IMS recovers metal units from the residues of industrial processing — dusts, powders, filtercakes, scales and sludges — and puts them back into production. Recovered units replace metal that would otherwise come from primary mining.")}
        align="split"
        action={
          <div className="flex flex-wrap gap-3">
            <Button href="/recycling" variant="primary">{p("Recovery capabilities")}</Button>
            <Button href="/about/sustainability" variant="secondary">{p("Our environmental position")}</Button>
          </div>
        }
      />
      ...
      <div className="mt-14 grid gap-10 lg:grid-cols-12 lg:gap-14">
        <div className="lg:col-span-7">
          ...
          <p className="mt-6 text-[0.875rem] text-steel-600">
            {p("{n} streams in total, from extremely fine metallic dusts to coarse turnings.", { n: recoveryStreams.length })}{" "}
            <ArrowLink href="/recycling">{p("See every stream")}</ArrowLink>
          </p>
        </div>
        ...
        <div className="lg:col-span-5">
          ...
          <blockquote className="mt-6 border-s-2 border-brand-700 ps-5">
            <p className="text-[0.9375rem] leading-relaxed text-steel-700">
              {p("Our recycling process enables us to revalorise thousands of tons of material destined for landfill every year. This not only reduces environmental impact and valuable landfill space, but replaces metal units originating from primary mining operations.")}
            </p>
            <footer className="mt-3 font-mono text-[0.6875rem] uppercase tracking-[0.12em] text-steel-500">
              {p("IMS environmental position")}
            </footer>
          </blockquote>
        </div>
      </div>
