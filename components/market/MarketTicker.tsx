...
export function MarketTicker({
  ...
  useEffect(() => {
    const unsubscribe = subscribeMarket(setData);
    void loadMarket();
    return unsubscribe;
  }, []);

  /* Manual refresh. The shared store notifies every subscriber, so pressing it
     on one bar updates the other bar and the board with it — refreshing one
     while its neighbours stayed stale would be worse than not offering it. */
  const refresh = async () => {
